...
struct pusher
{
    ...
    {
        static_assert(sizeof(T) == 0, "parameter types other than signed or unsigned char, short, int, long, std::string, bool, pointers, Coroutine, Userdata, Table or Function not available");
    }
};
...
struct popper
{
    ...
    {
        static_assert(sizeof(T) == 0, "returns types other than signed or unsigned char, short, int, long, std::string, bool, pointers, Coroutine, Userdata, Table or Function not available" );
    }
};
...
// CppInterface's types
DEFINE_TYPE_TEMPLATE_FOR(LightUserdata<TYPE>, typename TYPE, param.PushToStack(state.get()), LightUserdata<TYPE> res(state,-1))
DEFINE_TYPE_TEMPLATE_FOR(Userdata<TYPE>, typename TYPE, param.PushToStack(state.get()), Userdata<TYPE> res(state,-1))
DEFINE_TYPE_TEMPLATE_FOR(Function<SIG>, typename SIG, param.PushToStack(state.get()), Function<SIG> res(state, -1))
DEFINE_TYPE_TEMPLATE_FOR(Table,, param.PushToStack(state.get()), Table res(state,-1))
DEFINE_TYPE_TEMPLATE_FOR(Coroutine,, param.PushToStack(state.get()), Coroutine res(state,-1))
...
// pointers
DEFINE_TYPE_TEMPLATE_FOR(TYPE*, typename TYPE, lua_pushlightuserdata(state.get(), (void*)param), TYPE* res = LightUserdata<TYPE>(state,-1).GetPointer())
